...
void* func1(void* arg)
{
    ...
    /* for every pixel */
    struct mystruct  *range = (struct mystruct*)(arg);
    //printf("range from %d to %d \n",(*range).a, (*range).b);
    for( int px=(*range).a; px<(*range).b; ++px )
    {
        ...
        for( int py=0; py<height; ++py )
        {
             ...
             */
	    printarr[px][py][0] = scaled_color[0];
	    printarr[px][py][1] = scaled_color[1];
	    printarr[px][py][2] = scaled_color[2];  
        }
    }
    return NULL;
}
...
main( int argc, char **argv )
{
    ...
    */
    ...
    int *rt = (int*)malloc(sizeof(int)*(nthreads+1));
    struct mystruct* range = (struct mystruct*)malloc(sizeof(struct mystruct)*nthreads);
    //pthread_t threads[nthreads];
    //int thread_id[nthreads];
    
    if (threads == NULL || rt == NULL || range == NULL)
      {
    ...
    for (int k = 0; k < nthreads; k++)
      {
	    range[k].a = rt[k];
	    range[k].b = rt[k+1];
	    //	    printf("ranging from %d to %d \n",range[k].a, range[k].b);
      }
    for (int k = 0; k < nthreads; k++)
      {
	int check = pthread_create(&threads[k],NULL, func1,&range[k]);
        if (check != 0)
          {
            fprintf(stderr, "error in creating threads, error code: %d \n",check);
            exit(-1);
          }
      }
    ...
    for (int k = 0; k < nthreads; k++)
      {
	int  check = pthread_join(threads[k],NULL);
        if (check != 0)
          {
            fprintf(stderr,"error in joining threads, error code: %d \n", check);
            exit(-1);
          }
      }
	...
	for (int j = 0; j < height; j++)
	  {
	     printf( "%.0f %.0f %.0f\n", printarr[i][j][0], printarr[i][j][1], printarr[i][j][2] );
	  }
	printf("\n");
      }
